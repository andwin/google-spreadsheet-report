...
const appendData = async (data, options) => {
  ...
  options.worksheet = options.worksheet || ''
...
const makeSureSheetExists = (sheets, options) => {
  ...
  return new Promise((resolve, reject) => {
    if (!options.worksheet) return resolve(0)

    sheets.spreadsheets.get({ spreadsheetId }, (getErr, res) => {
      if (getErr) return reject(getErr)

      const found = res.data.sheets.find(s => s.properties.title === options.worksheet)
...
const purgeRows = (sheets, sheetId, options) => {
  ...
  return new Promise((resolve, reject) => {
    const request = {
      spreadsheetId,
      range: `${worksheet}!A2:A${numberOfRowsToCheck + 1}`,
    }
